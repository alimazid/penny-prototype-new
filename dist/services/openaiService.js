...
class OpenAIService {
    ...
    constructor() {
        ...
        if (!apiKey) {
            logger_1.logger.warn('OpenAI API key not configured. AI features will be disabled.');
        ...
        try {
            ...
            logger_1.logger.info('OpenAI service initialized successfully');
        }
        catch (error) {
            logger_1.logger.error('Failed to initialize OpenAI service:', error);
            this.isConfigured = false;
        }
    }
    get configured() {
     ...
     */
    async classifyEmail(subject, body, sender, accountId) {
        ...
        try {
            ...
            const prompts = await promptService_1.promptService.getClassificationPrompts(model, 'v1');
            // Render the user prompt with variables
            const userPrompt = promptService_1.promptService.renderPrompt(prompts.user, {
                sender,
                subject,
            ...
            if (accountId) {
                await database_1.OpenAIMetrics.recordAPICall(accountId, 'classification', model);
            }
            ...
            logger_1.logger.error('Error classifying email with OpenAI:', error);
     ...
     */
    async extractFinancialData(subject, body, category, accountId) {
        if (!this.isConfigured) {
            return this.fallbackExtraction(subject, body);
        }
        try {
            const model = process.env.AI_EXTRACTION_MODEL || 'gpt-4o-mini';
            const prompts = await promptService_1.promptService.getExtractionPrompts(model, 'v1');
            ...
            // Track the API call for the account
            if (accountId) {
                await database_1.OpenAIMetrics.recordAPICall(accountId, 'extraction', model);
            }
            const result = response.choices[0]?.message?.content;
            if (!result) {
                throw new Error('No response from OpenAI');
            ...
            logger_1.logger.debug('=== EMAIL EXTRACTION DEBUG ===');
            logger_1.logger.debug('Subject:', subject);
            logger_1.logger.debug('Body preview:', body.substring(0, 500) + '...');
            logger_1.logger.debug('Category:', category);
            logger_1.logger.debug('OpenAI raw response:', result.substring(0, 1000) + (result.length > 1000 ? '...' : ''));
            const extractedData = this.parseExtractionResult(result);
            // Log parsed extraction data
            logger_1.logger.debug('Parsed extraction data:', {
            ...
            logger_1.logger.error('Error extracting financial data with OpenAI:', error);
            logger_1.logger.warn('=== USING FALLBACK EXTRACTION ===');
            logger_1.logger.debug('Fallback extraction inputs - Subject:', subject);
            logger_1.logger.debug('Fallback extraction inputs - Body preview:', body.substring(0, 500) + '...');
            const fallbackData = this.fallbackExtraction(subject, body);
            logger_1.logger.debug('Fallback extraction result:', fallbackData);
            return fallbackData;
        }
    }
    /**
     ...
     */
    buildClassificationPrompt(subject, body, sender) {
     ...
     */
    buildExtractionPrompt(subject, body, category) {
        // Legacy method - keeping for backward compatibility
        // New code should use promptService
    ...
    parseClassificationResult(result) {
        try {
            ...
            logger_1.logger.error('Error parsing classification result:', error);
            logger_1.logger.error('Raw result:', result.substring(0, 500));
            return {
                isFinancial: false,
                confidence: 0,
                category: 'error',
    ...
    parseExtractionResult(result) {
        try {
            ...
            logger_1.logger.debug('Successfully parsed JSON from OpenAI response:', {
            ...
            logger_1.logger.error('Error parsing extraction result:', error);
            logger_1.logger.error('Raw result:', result.substring(0, 500));
            // Enhanced fallback for non-JSON responses - try to extract data from plain text
            logger_1.logger.warn('Attempting fallback text parsing for non-JSON response');
     ...
     */
    fallbackClassification(subject, body, sender) {
        ...
        if (isFinancial) {
            if (text.includes('bank') || text.includes('statement'))
                category = 'banking';
            else if (text.includes('credit') || text.includes('card'))
                category = 'credit_card';
            else if (text.includes('payment') || text.includes('purchase'))
                category = 'payment';
     ...
     */
    fallbackExtraction(subject, body) {
    ...
    validateCreditCardExtraction(data) {
        ...
        if (missingFields.length > 0) {
            logger_1.logger.info(`Credit card extraction missing ${missingFields.length} fields: ${missingFields.join(', ')} - adjusted confidence: ${adjustedConfidence.toFixed(2)}`);
        }
        return {
            ...data,
     ...
     */
    fallbackTextExtraction(text) {
        logger_1.logger.info('Using fallback text extraction for OpenAI response');
     ...
     */
    validateGeneralExtraction(data, subject, body, category) {
        let warningFlags = [];
        ...
        if (adjustedConfidence > 0.7 && extractedFieldCount < 3) {
            ...
            logger_1.logger.warn(`🚨 VALIDATION WARNING: High confidence (${data.confidence}) but only ${extractedFieldCount} fields extracted`);
        }
        ...
        if (data.amount && data.amount > 999999) {
            ...
            logger_1.logger.warn(`🚨 VALIDATION WARNING: Suspiciously high amount: ${data.amount}`);
        }
        ...
        if (data.currency && body.includes('RD$') && data.currency !== 'DOP') {
            ...
            adjustedConfidence = Math.min(adjustedConfidence, 0.6);
            logger_1.logger.warn(`🚨 VALIDATION WARNING: Currency mismatch - found RD$ but extracted ${data.currency}`);
        }
        ...
        if (data.merchantName && data.merchantName.length < 2) {
            ...
            adjustedConfidence = Math.min(adjustedConfidence, 0.5);
            logger_1.logger.warn(`🚨 VALIDATION WARNING: Invalid merchant name: "${data.merchantName}"`);
        }
        ...
        if (data.date && !data.date.match(/\d{4}-\d{2}-\d{2}/) && !data.date.match(/\d{1,2}\/\d{1,2}\/\d{4}/)) {
            ...
            adjustedConfidence = Math.min(adjustedConfidence, 0.6);
            logger_1.logger.warn(`🚨 VALIDATION WARNING: Suspicious date format: "${data.date}"`);
        }
        ...
        if (category === 'credit_card' && !subjectBodyText.includes('tarjeta') && !subjectBodyText.includes('card')) {
            ...
            logger_1.logger.warn(`🚨 VALIDATION WARNING: Category is credit_card but content doesn't mention cards`);
        }
        // Log all validation warnings
        if (warningFlags.length > 0) {
            logger_1.logger.warn('=== EXTRACTION VALIDATION WARNINGS ===');
            logger_1.logger.warn('Subject:', subject);
            logger_1.logger.warn('Category:', category);
            logger_1.logger.warn('Original confidence:', data.confidence);
            logger_1.logger.warn('Adjusted confidence:', adjustedConfidence);
            logger_1.logger.warn('Warning flags:', warningFlags);
            logger_1.logger.warn('Extracted data:', {
                amount: data.amount,
