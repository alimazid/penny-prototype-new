...
router.post('/start', async (req, res) => {
  try {
    ...
    // Start monitoring
    const result = await emailMonitoringService.startMonitoring(accountId);
    
    if (result.success) {
      logger.info(`Started monitoring account ${account.gmailAddress} via API`);
    }

    res.json(result);
    
  } catch (error) {
    logger.error('Error starting monitoring via API:', error);
    res.status(500).json({ 
      success: false, 
...
router.post('/stop', async (req, res) => {
  try {
    ...
    // Stop monitoring
    const result = await emailMonitoringService.stopMonitoring(accountId);
    
    if (result.success) {
      logger.info(`Stopped monitoring account ${accountId} via API`);
    }

    res.json(result);
    
  } catch (error) {
    logger.error('Error stopping monitoring via API:', error);
    res.status(500).json({ 
      success: false, 
...
router.post('/sync', async (req, res) => {
  try {
    ...
    // Trigger manual sync
    const result = await emailMonitoringService.triggerManualSync(accountId);
    
    if (result.success) {
      logger.info(`Manual sync triggered for account ${accountId} via API`);
    }

    res.json(result);
    
  } catch (error) {
    logger.error('Error triggering manual sync via API:', error);
    res.status(500).json({ 
      success: false, 
...
router.get('/status/:accountId', async (req, res) => {
  try {
    ...
    const isMonitoring = emailMonitoringService.isMonitoring(accountId);
    const monitoringSession = emailMonitoringService.getMonitoringSession(accountId);
...
router.get('/emails/:accountId', async (req, res) => {
  try {
    ...
    logger.error('Error fetching emails for account:', error);
    res.status(500).json({ 
      success: false, 
...
router.post('/settings', async (req, res) => {
  try {
    ...
    if (checkInterval && typeof checkInterval === 'number') {
      emailMonitoringService.setCheckInterval(checkInterval);
      logger.info(`Updated monitoring check interval to ${checkInterval}ms via API`);
    }
...
router.post('/process-email/:emailId', async (req, res) => {
  try {
    ...
    try {
      ...
      if (!emailBody) {
        logger.info(`Email ${emailId} missing bodyText, fetching from Gmail...`);
        ...
        try {
          ...
          logger.info(`Updated email ${emailId} with full bodyText (${emailBody.length} chars)`);
        } catch (gmailError) {
          logger.error(`Failed to fetch email content from Gmail for ${emailId}:`, gmailError);
      ...
      logger.info(`Successfully processed email ${emailId} - Classification: ${classificationEnum}`);
    ...
    logger.error('Error processing email manually:', error);
    res.status(500).json({
      success: false,
...
router.post('/process-pending/:accountId', async (req, res) => {
  try {
    ...
    logger.info(`Batch processing completed: ${processedCount}/${pendingEmails.length} emails processed`);
...
router.post('/process-stuck-emails', async (_req, res) => {
  try {
    ...
    logger.error('Error processing stuck emails:', error);
    res.status(500).json({
      success: false,
